...
impl GetCmd {
    ...
    pub fn exec(
        ...
        if let Some(topic_args) = app_matches.subcommand_matches("topics") {
                        ...
                        result_table
                            .write(
                                format!(
                                    ...
                                    partition.id(),
                                    partition.leader(),
                                    partition.isrs().iter().map(|i|i.to_string()).collect::<Vec<String>>().join(","),
                                    partition.replicas().iter().map(|i|i.to_string()).collect::<Vec<String>>().join(",")
                                )
                                .as_ref(),
                            )
